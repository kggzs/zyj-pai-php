...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='拍摄链接表';

-- ============================================
-- 3. 照片表（包含所有扩展字段，支持照片和视频，包含EXIF数据）
-- ============================================
...
CREATE TABLE IF NOT EXISTS `photos` (
  ...
  `exposure_mode` VARCHAR(50) DEFAULT NULL COMMENT '曝光模式',
  `white_balance` VARCHAR(50) DEFAULT NULL COMMENT '白平衡',
  `flash` VARCHAR(50) DEFAULT NULL COMMENT '闪光灯状态',
  -- EXIF数据字段（照片信息）
  `orientation` INT(11) DEFAULT NULL COMMENT '照片方向（1-8）',
  `width` INT(11) DEFAULT NULL COMMENT '照片宽度（像素）',
  `height` INT(11) DEFAULT NULL COMMENT '照片高度（像素）',
  `location_address` VARCHAR(500) DEFAULT NULL COMMENT '地理位置（地址）',
  `exif_data` TEXT DEFAULT NULL COMMENT '完整EXIF数据（JSON格式）',
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
